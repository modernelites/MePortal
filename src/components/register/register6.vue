    ...
    methods: {
      ...
      Course_List() {
        this.$http.get(this.ApiUrl + 'me/Course/Course_List?CourseTypeID=0').then((response) => {
          ...
          this.Course_Items_Fillter = this.Course_List_Fillter(6);
        }, function () {
          console.log('请求发送失败');
        });
      },
      ...
      submit() {
        let Sentiment = $('input[name=Sentiment]')[0];
        if (Sentiment.files.length) {
          this.Sentiment_Upload(this.CourseReg_Add);
        } else {
          this.CourseReg_Add();
        }
      },
